        ...
        try
        {
            return (IExecutionBuilderConfigurator)(Activator.CreateInstance(
                                                   ...
                                                   throw new InvalidOperationException(
                                                       $"Could not create mocker execution configurator '{configuratorType.FullName}'."));
        }
        catch (Exception exception)
        {
            if (configuratorType.Assembly == entryAssembly)
            {
                logger.LogError(exception,
                    "Failed to create mocker execution configurator {ConfiguratorType}",
                    configuratorType.FullName);
                throw;
            }

            logger.LogWarning(exception,
                "Skipping mocker execution configurator {ConfiguratorType} because it could not be created.",
                configuratorType.FullName);
            return null;
        }
    }
